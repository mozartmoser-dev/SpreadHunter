...
CHAVE = "white_list_box4p"
ESTRATEGIA = "BOX_4P"
DESCRICAO = "Ativos permitidos no monitoramento Box 4 Pontas"


def ler_whitelist(db_path: str | None = None) -> list[str]:
...
def salvar_whitelist(ativos: list[str], db_path: str | None = None):
    repo = ParametroRepository(db_path)
...
class WhitelistBox4PDialog(QDialog):
    def __init__(self, db_path: str | None = None, parent=None):
        super().__init__(parent)
        self.db_path = db_path
        self.setWindowTitle("Whitelist Box 4P")
    ...
    def _setup_ui(self):
        ...
        title = QLabel("Whitelist do Box 4 Pontas")
        title.setStyleSheet(
        ...
        desc = QLabel(
            "Apenas ativos nesta lista ser\u00e3o monitorados "
            "no Box 4P. Se vazia, todos os ativos ser\u00e3o exibidos."
        )
    ...
    def _carregar_lista(self):
        self.lista.clear()
        ativos = ler_whitelist(self.db_path)
    ...
    def _adicionar(self):
        ...
        if texto in itens:
            QMessageBox.information(self, "Aviso", f"'{texto}' j\u00e1 est\u00e1 na whitelist.")
    ...
    def _confirmar(self):
        ...
        salvar_whitelist(ativos, self.db_path)
        self.accept()
