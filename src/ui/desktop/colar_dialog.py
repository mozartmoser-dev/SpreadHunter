...
class ColarDialog(QDialog):
    ...
    def _plot_historico(self, ativo: str, preco_atual: float = None, strike_put: float = None, strike_call: float = None, n_sessoes: int = 21):
        ...
        # ── Linhas com rótulo central (tracejado interrompido, gradiente diagonal) ──
        if dates:
    ...
    def _abrir_pipeline(self):
        ...
        tracker = None
        parent = self.parent()
        if parent and hasattr(parent, '_worker') and hasattr(parent._worker, '_monitor_colares_uc'):
            tracker = getattr(parent._worker._monitor_colares_uc, '_ultimo_pipeline', None)
